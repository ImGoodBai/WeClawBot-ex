...
function resolveAccountsDir(): string {
  return path.join(resolveStateDir(), "clawbnb-weixin", "accounts");
}

/**
 * Path to the persistent get_updates_buf file for an account.
 * Stored alongside account data: ~/.openclaw/clawbnb-weixin/accounts/{accountId}.sync.json
 */
export function getSyncBufFilePath(accountId: string): string {
  return path.join(resolveAccountsDir(), `${accountId}.sync.json`);
}

/** Legacy single-account syncbuf (pre multi-account): `.clawbnb-weixin-sync/default.json`. */

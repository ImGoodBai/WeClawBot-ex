...
export const weixinPlugin: ChannelPlugin<ResolvedWeixinAccount> = {
  ...
  meta: {
    ...
    selectionLabel: "clawbnb-weixin (long-poll)",
    ...
    login: async ({ cfg, accountId, verbose, runtime }) => {
      ...
      if (waitResult.connected && waitResult.botToken && waitResult.accountId) {
        try {
          ...
          logger.info(
            `auth.login: saved account normalizedId=${normalizedId} userId=${redactToken(waitResult.userId)} bindingMode=${binding.mode} agentId=${binding.agentId} fallback=${binding.fallback} reloadMode=${binding.activation.mode} reloadTriggered=${binding.activation.triggered}`,
          );
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      aLog.info(
        `gateway.startAccount: accountId=${account.accountId} configured=${account.configured} enabled=${account.enabled} baseUrl=${account.baseUrl} cdnBaseUrl=${account.cdnBaseUrl}`,
      );
      ...
      if (!account.configured) {
        ...
        ctx.log?.error?.(
          `[${account.accountId}] weixin not logged in — run: openclaw channels login --channel clawbnb-weixin`,
        );
    ...
    loginWithQrWait: async (params) => {
      ...
      if (result.connected && result.botToken && result.accountId) {
        try {
          ...
          logger.info(
            `loginWithQrWait: saved account data for accountId=${normalizedId} userId=${redactToken(result.userId)} bindingMode=${binding.mode} agentId=${binding.agentId} fallback=${binding.fallback} reloadMode=${binding.activation.mode} triggered=${binding.activation.triggered}`,
          );
        } catch (err) {

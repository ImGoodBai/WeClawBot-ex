...
function getConfiguredUploadUrl(config: OpenClawConfig): string | undefined {
  const section = config.channels?.["clawbnb-weixin"] as { logUploadUrl?: string } | undefined;
  return section?.logUploadUrl;
}

/** Register the `openclaw clawbnb-weixin logs-upload` CLI subcommand. */
export function registerWeixinCli(params: { program: CliCommand; config: OpenClawConfig }): void {
  const { program, config } = params;

  const root = program.command("clawbnb-weixin").description("Weixin channel utilities");
  ...
  root
    ...
    .action(async (options: { url?: string; file?: string }) => {
      ...
      if (!uploadUrl) {
        console.error(
          `[weixin] No upload URL specified. Pass --url or set it with:\n  openclaw config set channels.clawbnb-weixin.logUploadUrl <url>`,
        );
        process.exit(1);
      }
